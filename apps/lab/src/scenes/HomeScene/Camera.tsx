...
const Scene = ["/", "/experiments"] as const;

type Scene = typeof Scene[number];
...
export function Camera() {
  ...
  useEffect(() => {
    const newRig = Rigs[path as Scene];
    if (groupRef.current && rig == undefined && newRig != undefined) {
  ...
  useFrame(({ mouse }, dt) => {
    ...
    if (position && rotation && groupRef.current) {
